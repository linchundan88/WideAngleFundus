...
for model_name in ['inception_resnet_v2', 'xception', 'inception_v3']:
    ...
    ds_train = Dataset_CSV(csv_or_df=csv_train, multi_labels=True, imgaug_iaa=iaa, image_shape=image_shape)
    loader_train = DataLoader(ds_train, batch_size=batch_size_train, shuffle=True,
                              num_workers=num_workers)
    ds_valid = Dataset_CSV(csv_or_df=csv_valid, multi_labels=True, image_shape=image_shape)
    loader_valid = DataLoader(ds_valid, batch_size=batch_size_valid,
                              num_workers=num_workers)
    ds_test = Dataset_CSV(csv_or_df=csv_test, multi_labels=True, image_shape=image_shape)
